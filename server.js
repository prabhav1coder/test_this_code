...
// Route to handle image upload and sending it to the Flask server
app.post('/process-image', upload.single('imageInput'), async (req, res) => {
    try {
      // Send the uploaded image to Flask server
      const imagePath = path.join(__dirname, 'uploads', req.file.filename);
      const flaskResponse = await axios.post('http://localhost:5000/generate-image', {
        image: fs.readFileSync(imagePath, { encoding: 'base64' })
      });
  
      // Respond with the generated image from Flask
      res.contentType('image/png');
      res.send(Buffer.from(flaskResponse.data, 'base64'));
    } catch (error) {
